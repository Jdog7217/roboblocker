...
class admin(commands.Cog):
    ...
    async def setup(self,interaction: discord.Interaction):
        ...
        server = await backend.newserver(interaction.guild.id)
        if server == 0:
    ...
    async def all(self, interaction: discord.Interaction) -> None:
        """ Gets all settings """
        server = await backend.getserver(interaction.guild.id)
        if server:
    ...
    async def channel(self, interaction: discord.Interaction,channel:discord.TextChannel=None) -> None:
        ...
        if channel:
            try:
                ...
                tr = False
            if tr:
                change = await backend.changesettings(interaction.guild.id,"channel",channel.id)
        else:
            change = await backend.changesettings(interaction.guild.id,"channel",None)
        if tr:
            if change ==1:
                if channel:
    ...
    async def role(self, interaction: discord.Interaction,role:discord.Role=None) -> None:
        ...
        if tr:
            if role:
                change = await backend.changesettings(interaction.guild.id,"role",role.id)
            else:
                change = await backend.changesettings(interaction.guild.id,"role",None)
            if change ==1:
                if role:
    ...
    async def activate(self, interaction: discord.Interaction,bot_on:bool) -> None:
        ...
        change = await backend.changesettings(interaction.guild.id,"disabled",bot_on)
        if change ==1:
